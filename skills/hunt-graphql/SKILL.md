...
### Modern GraphQL techniques (2024-2026)

1. **APQ "safelist illusion" bypass (Automatic Persisted Queries)** — When a server uses APQ, send only a `sha256Hash`; on `PersistedQueryNotFound`, resend with the full arbitrary `query` string + that hash and the server executes it AND caches it — APQ is a bandwidth optimization, not a safelist, so you run introspection/mutations even where "only persisted operations are allowed." True safelisting requires APQ disabled. Confirm: register an arbitrary op then replay it by hash. (Distinct from the persisted-query fallback bullet above — here APQ is *on* and you register-then-replay through it.) Source: https://www.apollographql.com/docs/apollo-server/performance/apq
2. **Nested-resolver BOLA** — Authorization is enforced only at the top-level/gateway resolver, but a NESTED type's resolver dereferences an attacker-supplied id without re-checking ownership. Reach a victim object by embedding it as a nested selection under an object you legitimately own (`myOrder{ items{ user(id:$victim){ email } } }` or `me{ organization(id:$other){ members } }`) rather than querying the protected root field. The bug is WHERE the check runs; nested paths bypass a root guard. Source: https://infosecwriteups.com/graphql-security-how-i-found-and-exploited-critical-idor-and-authorization-bypass-in-a-42ab78e13642
3. **Opaque-cursor decode → cross-type IDOR / enumeration** — Relay-style `after:`/`before:` cursors and node IDs are usually base64 of `{"type":"User","id":123}` or `arrayconnection:offset`. Base64-decode, mutate the embedded type/id/offset, re-encode, resubmit to enumerate other objects, jump object types, or page past authz-scoped windows. Cursors read as "opaque" and get ignored — a reliable object-ID leak+tamper channel. Source: https://www.apollographql.com/docs/react/pagination/cursor-based
4. **`@defer` / `@stream` incremental-delivery abuse** — servers supporting the incremental-delivery directives (Apollo Server 4+, graphql-js 17, Hot Chocolate) open two classes: (a) **DoS** — a query with hundreds of `... @defer` fragments or a `@stream(initialCount:0)` over a huge list forces the server to hold and multiplex many partial responses (each its own resolver+serialization), sidestepping depth/complexity limits that only score the *static* query; (b) **authz-filter bypass** — a response-level scrubber that redacts fields on the FINAL payload may leak them in an earlier `multipart/mixed`/`incremental` chunk, because deferred fragments are resolved and streamed before the top-level filter runs. Confirm: `Accept: multipart/mixed; deferSpec=20220824`, watch each chunk. Source: https://github.com/graphql/graphql-over-http
5. **Directive / alias overloading DoS (distinct from depth & field-dup)** — repeat a directive thousands of times on one field (`user @include(if:true) @include(if:true) ...×10000`) or overload thousands of unique aliases on one cheap field; many parsers/validators process every directive/alias in O(n) BEFORE any depth or cost limit applies, so a *shallow* query still exhausts CPU/memory. Amplification that depth-limiting and query-cost analysis both miss. Confirm on a low-load window; stop at a measured latency spike, never a takedown. Source: https://github.com/dolevf/graphql-cop
6. **GraphQL CSRF via non-preflighted content-type + `operationName` selection** — (a) POST the query as `Content-Type: application/x-www-form-urlencoded` or `text/plain` (a "simple request" → no CORS preflight) with `query=...`; many servers still parse it as GraphQL, so a state-changing mutation fires cross-site with the victim's cookies — stronger than the GET-mutation CSRF, and it works even when only POST is accepted. (b) Multi-operation documents: send two named operations and steer with the `operationName` param — if the server authorizes based on one operation but executes the one named in `operationName`, you bypass the guard. Confirm: mutation via `text/plain` from an off-origin page; and a 2-op doc where `operationName` selects the privileged op. Source: https://portswigger.net/web-security/csrf
7. **Injection smuggled through `variables` (WAF sees only the `query` string)** — edge WAFs and cost analyzers frequently inspect the `query` template but NOT the JSON `variables` map. Move the SQLi/NoSQL/SSTI/path payload out of an inline argument and into a `$var` passed via `variables` (`{"query":"query($q:String!){search(term:$q){id}}","variables":{"q":"' OR 1=1--"}}`) — the resolver concatenates it into the sink all the same. Also try type-confusion in variables (send an object/array where a scalar is expected → NoSQL operator injection). The WAF-bypass twin of the REST "SQLi in JSON values" angle. Source: https://cheatsheetseries.owasp.org/cheatsheets/GraphQL_Cheat_Sheet.html
